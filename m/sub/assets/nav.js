document.write("<nav>");
document.write("<ul>");
    
document.write("<li><a href='index.html' class = 'statement'>");
document.write("ABOUT</a></li>");
document.write("<li><a href = 'https://www.artsteps.com/' target='_blank' class = 'statement'>");
document.write("ARTSTEPS</a></li>");
document.write("<li><a href = 'https://www.instagram.com/ target='_blank' class = 'statement'>");
document.write("INSTAGRAM</a></li><br>");
document.write("<li class = 'line'>–––––––</li>");
document.write("<li class = 'line'>–––––––</li><br>");


document.write("<li><a href='江明昱.html' class = 'item'>");
document.write("<i>Light</i>, 江明昱</a></li>");
document.write("<li><a href='嚴紹恩.html' class = 'item'>");
document.write("<i>抽象危險犯</i>, 嚴紹恩</a></li>");
document.write("<li><a href='曾致諺.html' class = 'item'>");
document.write("<i>???</i>, 曾致諺</a></li>");
document.write("<li><a href='連亞珏.html' class = 'item'>");
document.write("<i>SAVE: 救亡圖存</i>, 連亞珏</a></li>");
document.write("<li><a href='何昀.html' class = 'item'>");
document.write("<i>Rats with Wings</i>, 何昀</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Toto the Hero</i>, 陳O卉</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>The Monk and the Fish</i>, 張O瑋</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Encounters at the End</i>, 吳O恩</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Mindgame</i>, 李O瑩</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Uncut Gems</i>, 張O淮</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Shirkers</i>, 謝O琪</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Daisies</i>, 黃O琪</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Mon Oncle</i>, 劉O如</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Woman in the Dunes</i>, 徐O穎</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Requiem for a Dream</i>, 蕭O宇</a></li>");
document.write("<li><a href='#' class = 'item'>");
document.write("<i>Brazil</i>, 連O珏</a></li><br>");

document.write("<li class = 'headernoA'>.............</li><br>");
        
document.write("<li class = 'headernoA'>歡迎蒞臨<br>109-2 線上期末展 :)</li><br><br>");

document.write("</ul>");
document.write("</nav>");
